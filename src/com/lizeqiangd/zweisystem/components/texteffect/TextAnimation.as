package com.lizeqiangd.zweisystem.components.texteffect
{
	 ...
	 * 2015.03.18 取消依赖animationevent.只依赖tweenlite
	 */
	public class TextAnimation
	{
		...
		private static var _animations:uint = 0;
		private static var _hasListener:Boolean = false
		...
		private static function enterframe(e:Event = null):void
		{
			...
			for (var i:int = 0; i < _typingTextArray.length; i++)
			{
				...
				{
					_typingTextArray[i].textField.dispatchEvent(new Event(Event.COMPLETE))
			...
			for (i = 0; i < _changeTextArray.length; i++)
			{
				...
				{
					_changeTextArray[i].textField.dispatchEvent(new Event(Event.COMPLETE))
