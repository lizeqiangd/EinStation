package com.lizeqiangd.zweisystem.applications.Weibo.MainController
{
	...
	public class WeiboMainController extends TitleWindows implements iApplication
	{
		...
		private function onMouseMaskClick(e:MouseEvent):void
		{
			...
			{
				isMask = false
				AnimationManager.fade(mc_bg, 0.2)
				AnimationManager.fade_out(btn_oauth)
				AnimationManager.fade_out(btn_publish)
			}
			else
			{
				isMask = true
				AnimationManager.fade_in(btn_publish)
				AnimationManager.fade(mc_bg, 0.7)
				AnimationManager.fade_in(btn_oauth)
			}
		}
