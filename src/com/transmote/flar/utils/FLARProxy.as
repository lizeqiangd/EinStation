...
package com.transmote.flar.utils {
	...
	public class FLARProxy extends Sprite implements IFLARSource {
		...
		public function activate () :void {
			if (this.stage) {
				this.mouseArea.addEventListener(MouseEvent.MOUSE_DOWN, this.onMouseDownEvent, false, 0, true);
				this.stage.addEventListener(KeyboardEvent.KEY_DOWN, this.onKeyDownEvent, false, 0, true);
				this.mouseArea.addEventListener(MouseEvent.MOUSE_UP, this.onMouseUpEvent, false, 0, true);
			} else {
		...
		public function deactivate () :void {
			if (this.stage) {
				this.mouseArea.removeEventListener(MouseEvent.MOUSE_DOWN, this.onMouseDownEvent);
				this.mouseArea.removeEventListener(MouseEvent.MOUSE_UP, this.onMouseUpEvent);
				this.stage.removeEventListener(KeyboardEvent.KEY_DOWN, this.onKeyDownEvent);
			}
		}
		...
		private function addMarker (patternId:uint, x:Number, y:Number) :void {
			var proxyMarker:FLARMarker = new FLARMarker(this.createProxyResult(patternId, x, y), this.createProxyMatrix(x, y), this);
		...
		private function updateMarker (patternId:uint, x:Number, y:Number) :void {
			...
			var updatedMarker:FLARMarker = new FLARMarker(this.createProxyResult(patternId, x, y), this.createProxyMatrix(x, y), this);
		...
		private function removeMarker (patternId:uint, x:Number, y:Number) :void {
			...
			var removedMarker:FLARMarker = new FLARMarker(this.createProxyResult(patternId, x, y), this.createProxyMatrix(x, y), this);
		...
		private function onMouseDownEvent (evt:MouseEvent) :void {
			this.mouseArea.addEventListener(MouseEvent.MOUSE_MOVE, this.onMouseMoveEvent);
			this.mouseIsDown = true;
			this.addMarker(this.activePatternId, evt.localX, evt.localY);
		}
		
		private function onMouseMoveEvent (evt:MouseEvent) :void {
			this.updateMarker(this.activePatternId, evt.localX, evt.localY);
		}
		
		private function onMouseUpEvent (evt:MouseEvent) :void {
			this.mouseArea.removeEventListener(MouseEvent.MOUSE_MOVE, this.onMouseMoveEvent);
			this.mouseIsDown = false;
			this.removeMarker(this.activePatternId, evt.localX, evt.localY);
		}
		
		private function onKeyDownEvent (evt:KeyboardEvent) :void {
